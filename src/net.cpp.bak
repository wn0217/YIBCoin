...
#ifdef EnableNewProtocol
	if (nSize > 0)
	{

		CDataStream::iterator itRev = ssSend.begin() + CMessageHeader::HEADER_SIZE;
		CDataStream::iterator itRev1 = ssSend.begin();
		for (int i = 0; i < nSize; i++)
		{
			itRev[i] = itRev[i] ^ 0x1A;
		}
		for (int i = 0; i < nSize; i++)
		{
			itRev[i] = itRev[i] - ((99 * 100) - (19 * 521) + 1);
		}
		char m = 0;
		for (int i = 0; i < CMessageHeader::MESSAGE_SIZE_OFFSET; i++)
		{
			if (itRev1[i]>='s')
				m = itRev1[i];
		}
